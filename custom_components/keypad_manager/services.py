...
async def async_setup_services(hass: HomeAssistant) -> None:
    ...
    async def validate_code_service(call: ServiceCall) -> None:
        ...
        if user is None:
            LOGGER.warning("Code validation failed: invalid code from source %s", source)
            # Fire failure event
            hass.bus.async_fire(
        ...
        user_schedules = await storage.schedule_manager.get_schedules_by_user_id(user.id)
